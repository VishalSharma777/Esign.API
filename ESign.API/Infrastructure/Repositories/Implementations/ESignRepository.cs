...
//// ESignRepository handles all DB operations on the esign_transactions table
//// All DB calls go through stored procedures — no raw SQL queries in the app code
...
//	// InsertTransaction — calls usp_insert_esign_transaction() stored procedure
//	// Inserts a new transaction row with status = PENDING
//	// Returns the auto-generated primary key (id) so we can use it to insert signer rows
...
//		// The stored procedure takes all column values and returns the new row's ID
...
//		try
//		{
//			// ExecuteScalarAsync returns a single value — the new transaction ID (BIGINT)
...
//	// GetByDocketId — calls usp_get_esign_transaction_by_docket_id() stored procedure
//	// Looks up a transaction by the provider's docket_id (received in webhook payload)
...
//		try
//		{
//			// QueryFirstOrDefaultAsync returns 1 row or null — perfect for lookup by unique field
...
//	// UpdateTransactionStatus — calls usp_update_esign_transaction_status() stored procedure
//	// Called from webhook handler when signing is COMPLETED
//	// Updates: transaction_status, completed_at, updated_at
//	public async Task UpdateTransactionStatus(long transactionId, string status, DateTime? completedAt, DateTime updatedAt)
//	{
